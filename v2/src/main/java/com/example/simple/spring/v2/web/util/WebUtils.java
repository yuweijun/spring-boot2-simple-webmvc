...
public abstract class WebUtils {
     ...
     * and throws a FileNotFoundException if the path cannot be resolved to a resource (in contrast to ServletContext's <code>getRealPath</code>, which returns null).
     *
    ...
    /**
     * Check the given request for a session attribute of the given name. Returns null if there is no session or if the session has no such attribute. Does not create a new session
     * if none has existed before!
     *
     * @param request current HTTP request
    ...
    /**
     * Check the given request for a session attribute of the given name. Throws an exception if there is no session or if the session has no such attribute. Does not create a new
    ...
    /**
     * Set the session attribute with the given name to the given value. Removes the session attribute if value is null, if a session existed at all. Does not create a new session
     * if not necessary!
     *
     * @param request current HTTP request
     * @param name    the name of the session attribute
    ...
    /**
     * Get the specified session attribute, creating and setting a new attribute if no existing found. The given class needs to have a public no-arg constructor. Useful for
     ...
     * this means that the HttpSessionMutexListener needs to be defined in <code>web.xml</code>. Falls back to the HttpSession itself if no mutex attribute found.
     * <p>The session mutex is guaranteed to be the same object during
     * the entire lifetime of the session, available under the key defined by the <code>SESSION_MUTEX_ATTRIBUTE</code> constant. It serves as a safe reference to synchronize on for
     * locking on the current session.
     * <p>In many cases, the HttpSession reference itself is a safe mutex
     * as well, since it will always be the same object reference for the same active logical session. However, this is not guaranteed across different servlet containers; the only
     * 100% safe way is a session mutex.
     *
     ...
     * @return the matching request object, or <code>null</code> if none of that type is available
     */
     ...
     * @return the matching response object, or <code>null</code> if none of that type is available
     */
    @SuppressWarnings("unchecked")
     ...
     * request attribute. Could check any request attribute that is only present in an include request.
     *
    ...
    /**
     * Expose the current request URI and paths as {@link HttpServletRequest} attributes under the keys defined in the Servlet 2.4 specification, for containers that implement 2.3
     * or an earlier version of the Servlet API:
    ...
    /**
     * Expose the Servlet spec's error attributes as {@link HttpServletRequest} attributes under the keys defined in the Servlet 2.3 specification, for error pages that are
     * rendered directly rather than through the Servlet container's error page resolution:
    ...
    /**
     * Clear the Servlet spec's error attributes as {@link HttpServletRequest} attributes under the keys defined in the Servlet 2.3 specification:
     * <code>javax.servlet.error.status_code</code>,
     * <code>javax.servlet.error.exception_type</code>,
     * <code>javax.servlet.error.message</code>,
     * <code>javax.servlet.error.exception</code>,
